...
export default async function ParentRootPage({ params }: { params: Promise<{ schoolName: string }> }) {
    ...
    if (userRes.success && userRes.data) {
        ...
        if (familyRes.success && familyRes.students) {
            ...
            if (students.length === 1 && students[0].type === "STUDENT") {
                redirect(`/${schoolName}/parent/${user.id}/${students[0].id}?phone=${phone}`);
            }
        }

        // Default: Redirect to dashboard
        redirect(`/${schoolName}/parent/${user.id}?phone=${phone}`);
    }
