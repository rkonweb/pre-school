...
export async function sendNotificationToGroup(userIds: string[], title: string, body: string, data?: any, priority: "high" | "normal" = "normal") {
    try {
        // 1. Find all active device tokens for these users
        const tokens = await prisma.deviceToken.findMany({
        ...
        console.error("FCM Group Send Error:", error);
        return { success: false, error: error.message };
    }
}
