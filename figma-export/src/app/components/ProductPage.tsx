        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
            {appEcosystem.map((app, index) => {
              const Icon = app.icon;
              const isParentApp = app.name === "Parent App";
              return (
                <motion.button
                  key={app.name}
                  onClick={() => isParentApp && navigate("/parent-app")}
                  className={`bg-slate-800/50 backdrop-blur-sm border border-teal-400/20 rounded-3xl p-8 hover:border-teal-400/40 transition-all group text-left ${
                    isParentApp ? "cursor-pointer hover:bg-slate-800/70" : ""
                  }`}
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                ...
                >
                  ...
                  <p className="text-slate-300 mb-3">{app.description}</p>
                  {isParentApp && (
                    <span className="text-sm text-teal-400 font-semibold flex items-center gap-2">
